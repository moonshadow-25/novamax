...
def main():
    ...
    uv_lock = os.path.join(install_root, 'uv.lock')

    if os.path.exists(uv_lock):
        print("  [INFO] 检测到 uv.lock，重新导出 requirements.txt...")
        export_result = run(
            ['uv', 'export', '--frozen', '--no-hashes', '--extra', 'webui', '-o', requirements],
            cwd=install_root,
            check=False,
        )
        if export_result.returncode != 0:
            raise RuntimeError('uv export 失败，无法从 uv.lock 导出 requirements.txt')
