...
function TtsSettingsDrawer({ visible, model, onClose, onSave, onDelete }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      message.success(t('settingsDrawer.ttsConfigSaved'));
      onSave?.();
    } catch (e) {
      message.error(t('settingsDrawer.saveFailed') + ': ' + (e?.response?.data?.error || e?.message || ''));
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <>
      <Drawer
        ...
        extra={
          <Popconfirm title={t('settingsDrawer.deleteCard')} description={t('settingsDrawer.deleteCardAndFilesDesc')}
            okText={t('settingsDrawer.delete')} okButtonProps={{ danger: true }} cancelText={t('settingsDrawer.cancel')} onConfirm={handleDelete}>
        ...
        footer={
          <Space style={{ justifyContent: 'flex-end', width: '100%' }}>
            ...
            <Button type="primary" loading={saving} onClick={handleSave}>{t('settingsDrawer.save')}</Button>
          </Space>
        }
      >
        <Space direction="vertical" size={16} style={{ width: '100%' }}>
          ...
          <div>
            ...
            {!engineInstalled ? (
              <Alert type="warning" showIcon message={t('settingsDrawer.ttsEngineNotInstalled')} style={{ marginTop: 8 }}
                action={<Button size="small" type="primary" onClick={() => setShowEngineModal(true)}>{t('settingsDrawer.installTtsEngine')}</Button>} />
          ...
          <Button icon={<FolderOpenOutlined />} onClick={async () => {
            try { await backendService.openLogsFolder(); message.success(t('settingsDrawer.logsFolderOpened')); }
            catch { message.error(t('settingsDrawer.openFailed')); }
          }} block>{t('settingsDrawer.openLogsFolder')}</Button>
        </Space>
      </Drawer>

      <EngineDownloadModal visible={showEngineModal} engineId="tts" engineInfo={engineInfo}
        onComplete={async () => { setShowEngineModal(false); await refreshEngineStatus(); message.success(t('settingsDrawer.ttsEngineInstalled')); }}
