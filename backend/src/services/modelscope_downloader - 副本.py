...
def get_model_files(model_id, revision='master', root=''):
    """获取模型文件列表，root 为空时自动尝试 '' 和 'models'"""
    url = f"{BASE_URL}/api/v1/models/{model_id}/repo/files"

    # 如果指定了 root 就直接用，否则自动探测
    roots_to_try = [root] if root else ['', 'models']

    for rt in roots_to_try:
        params = {'Revision': revision, 'Recursive': 'true', 'Root': rt}
        print(f"请求 API: {url}?Revision={revision}&Recursive=true&Root={rt}", file=sys.stderr, flush=True)
        try:
            ...
            if files:
                print(f"获取到 {len(files)} 个文件 (Root={rt})", file=sys.stderr, flush=True)
                return [f for f in files if not f.get('IsDir', False)]
            print(f"Root={rt} 返回 0 个文件，尝试下一个...", file=sys.stderr, flush=True)
        except Exception as e:
            print(f"Root={rt} 获取文件列表失败: {e}", file=sys.stderr, flush=True)

    print("所有 Root 均失败", file=sys.stderr, flush=True)
    return []


def match_patterns(filename, patterns):
    """检查文件名是否匹配任意一个 pattern（支持 fnmatch 通配符和文件夹前缀）"""
    if not patterns:
        return True
    basename = os.path.basename(filename)
    for pattern in patterns:
        # 文件夹前缀匹配：pattern 以 /* 结尾，匹配该目录下所有文件（含子目录）
        if pattern.endswith('/*'):
            prefix = pattern[:-1]  # 去掉 *，保留末尾 /
...
def download_file(model_id, file_name, file_path_in_repo, output_dir, revision='master', preserve_paths=False):
    """
    下载单个文件，使用 .part 文件。支持断点续传。
    preserve_paths=True 时保留仓库原始目录结构。
    """
    if preserve_paths:
        # 保留仓库路径：{output_dir}/{file_path_in_repo}
        target = os.path.join(output_dir, file_path_in_repo)
    else:
        # 兼容旧行为：仅使用文件名
    ...
    try:
        ...
        if resp.status_code == 404:
            print(f"⊘ 文件不存在 (404)，跳过: {file_name}", file=sys.stderr, flush=True)
            return True
...
def download_model(model_id, output_dir, files=None, revision='master', preserve_paths=False):
    """
    ...
    Args:
        ...
        output_dir: 输出目录
        files: 要下载的文件模式列表，None 表示下载所有文件。
        revision: 版本/分支，默认 'master'
        preserve_paths: True 保留仓库原始目录结构
    """
    try:
        ...
        for f in files_to_download:
            ...
            if not file_name:
                continue
            try:
                download_file(model_id, file_name, file_path_in_repo, output_dir, revision, preserve_paths)
            except Exception as e:
                failed_count += 1
                print(f"⊘ 下载失败跳过: {file_name} — {e}", file=sys.stderr, flush=True)
        if failed_count > 0:
            print(f"共 {failed_count} 个文件下载失败（已跳过），其余正常", file=sys.stderr, flush=True)
...
def main():
    ...
    parser.add_argument('--preserve-paths', action='store_true', help='Preserve repo directory structure')
